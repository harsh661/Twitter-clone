...
const Sidebar = () => {
  ...
  return (
    <div className={`${darkMode && 'bg-black text-white'} flex flex-row justify-around phone:justify-normal w-full phone:w-min absolute bottom-0 phone:flex-col xl:w-64 h-min phone:h-[100dvh] gap-3 xl:pr-10 phone:p-3 phone:pt-0 p-2 phone:relative`}>
        ...
        <div className={`hidden phone:flex items-center xl:justify-between justify-center absolute bottom-0 left-0 right-0 rounded-full w-full p-3 xl:p-3 mb-2 ${darkMode ? 'hover:bg-hover text-white':'hover:bg-grey'}`}>
            <span className='flex items-center gap-3'>
                <Avatar size={8} url={user?.avatar}/>
                <b className='hidden xl:block'>{user?.name}</b>
            </span>
            <span className='hidden xl:block w relative cursor-pointer'>
                ...
                <div className={`${card ? 'flex': 'hidden'} ${darkMode ? 'card-light bg-black text-white': 'card'} flex-col absolute w-60 -top-48 right-0 gap-2 p-3`}>
                    ...
                    <select className='bg-transparent' name="theme" id="theme">
                        <option value="false" onClick={()=>{setDarkMode(false); setCard(false)}}>Default</option>
                        <option value="true" onClick={()=>{setDarkMode(true); setCard(false)}}>Lights out</option>
                    </select>
                </div>
            </span>
        </div>
